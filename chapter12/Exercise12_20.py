...
'''(Tkinter: Display an n-sided regular polygon) Define a subclass of Canvas,
...
heptagon, and octagon from RegularPolygonPanel and display them, as
...
class MainClass:
    def __init__(self):
        ...
        canvas.pack(side=LEFT)
        canvas = RegularPolygonCanvas(window, 9)
        canvas.pack(side=LEFT)
        canvas = RegularPolygonCanvas(window, 10)
        canvas.pack(side=LEFT)
