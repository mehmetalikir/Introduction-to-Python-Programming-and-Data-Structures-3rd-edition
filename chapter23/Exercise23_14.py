...
'''(Display a minimum spanning tree) Revise GraphView in Listing 22.4
to display a weighted graph and a minimum spanning tree for the graph in
Figure 23.1, as shown in Figure 23.19. The edges in the MST are shown in red.'''
...
class GraphView(Canvas):
    ...
    def drawGraph(self):
        ...
        tree = self.graph.getMinimumSpanningTree()
        searchOrders = tree.getSearchOrders()
        print(searchOrders)
        for i in searchOrders:
            if tree.getParent(i) >= 0:
                x1 = self.graph.getVertex(i).getX()
                y1 = self.graph.getVertex(i).getY()
                x2 = self.graph.getVertex(tree.getParent(i)).getX()
                y2 = self.graph.getVertex(tree.getParent(i)).getY()
...
window.title("Exercise23_14")  # Set title
