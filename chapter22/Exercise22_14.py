...
'''(Hamiltonian cycle) The Hamiltonian path is a path that visits every vertex in the
graph exactly once. If there is an edge from ending vertex to the starting
vertex in the that path, the Hamiltonian path is called a Hamiltonian cycle
VI.E. Add the following getHamiltonianCycle function in the
Graph class:
# Return a Hamiltonian cycle
# Return None if the graph doesn't contain a Hamiltonian cycle

    def getHamiltonianCycle()
...
class Edge:
    ...
    def getHamiltonianCycle(self):
        ...
        def backtrack(vertex, pos):
            if pos == num_vertices:
                # Check if the last vertex has an edge to the starting vertex
                if self.has_edge(vertex, path[0]):
                    path[pos] = path[0]
                    return True
                else:
                    return False

            for next_vertex in self.adjacent_vertices(vertex):
                if not visited[next_vertex]:
                    path[pos] = next_vertex
                    visited[next_vertex] = True

                    # Recursively check the next vertex
                    if backtrack(next_vertex, pos + 1):
                        return True

                    # Backtrack if the next vertex doesn't lead to a solution
                    visited[next_vertex] = False

            return False

        # Start the backtracking from the second vertex
        if backtrack(0, 1):
            return path
        else:
            return None
