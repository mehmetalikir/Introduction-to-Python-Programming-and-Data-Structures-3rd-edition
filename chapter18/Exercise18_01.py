...
'''(Implement set operations in LinkedList) Define a new class named
MyLinkedList that extends LinkedList with the following set methods:
...
class MyLinkedList(LinkedList):
    # Function to add all elements from another list to the end of this list
    def addAll(self, otherList):
        current = otherList._LinkedList__head
        while current is not None:
            self.addLast(current.element)
            current = current.next

    # Function to remove all elements from this list that are also present in another list
    def removeAll(self, otherList):
        current = otherList._LinkedList__head
        while current is not None:
    ...
    # Function to retain only the elements in this list that are also present in another list
    def retainAll(self, otherList):
        current = self._LinkedList__head
        while current is not None:
            if not otherList.contains(current.element):
                temp = current
                current = current.next
                self.remove(temp.element)
            else:
                current = current.next

    # Function to get the element at the specified index in this list
    def get(self, index):
        if index < 0 or index >= self.getSize():
            return None
        else:
    ...
    # Function to get the last index of the specified element in this list
    def lastIndexOf(self, e):
        current = self._LinkedList__head
        lastIndex = -1
        index = 0
    ...
    # Function to set the element at the specified index in this list to a new element
    def set(self, index, e):
        if index < 0 or index >= self.getSize():
            return None
        else:
            ...
            current.element = e
def main():
    lista = input("Please enter list1: ").split()  # red green red black
    listb = input("Please enter list2: ").split()  # red black yellow yellow
